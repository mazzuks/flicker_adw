    ...
    <div className="fixed inset-0 bg-adworks-dark/80 backdrop-blur-md z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden border border-white/20">
        <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-adworks-gray/30">
          <div>
            <span className="bg-adworks-blue text-white px-3 py-1 rounded-full text-[9px] font-black tracking-widest uppercase">Operacional</span>
            <h2 className="text-2xl font-black text-adworks-dark mt-2 italic uppercase tracking-tighter">Novo Processo</h2>
          </div>
          <button onClick={onClose} className="w-12 h-12 rounded-xl bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-red-500 transition-all">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Tipo de Serviço</label>
              <select 
                value={formData.type}
          ...
          <div>
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Notas Iniciais (Opcional)</label>
            <textarea 
