import { X, CheckCircle2, FileText, MessageSquare, History, ChevronRight, Calendar } from 'lucide-react';
...
export function DealDrawer({ dealId, isOpen, onClose, dealData }: DealDrawerProps) {
  const [activeTab, setActiveTab] = useState<'checklist' | 'docs' | 'messages' | 'audit'>('checklist');
    ...
    <>
      ...
      >
        <header className="p-6 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
               <Badge variant="info" className="bg-blue-50 text-blue-600 border-blue-100 font-black text-[10px] tracking-widest uppercase italic">
                 {dealData?.stage_key || 'ETAPA'}
               </Badge>
               <span className="text-slate-300">/</span>
               <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">{dealData?.company_name || 'EMPRESA'}</span>
            </div>
            <div className="flex items-center gap-2">
               <button onClick={onClose} className="p-2 hover:bg-red-50 rounded-lg text-slate-400 hover:text-red-500 transition-all">
                  <X className="w-5 h-5" />
               </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-2 text-slate-500">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest">SLA: {dealData?.sla_status === 'breached' ? 'ATRASADO' : 'EM DIA'}</span>
             </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-[#FDFDFD]">
          {activeTab === 'checklist' && (
            <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
               <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 italic">Fila de Execução</h4>
               <div className="space-y-2">
                  {[
                    { id: '1', title: 'Validar documentos dos sócios', done: true },
                    { id: '2', title: 'Consultar viabilidade do endereço', done: false },
                  ].map((item) => (
                    <div key={item.id} className="flex items-center gap-3 p-4 bg-white border border-slate-100 rounded-xl hover:border-blue-200 transition-all cursor-pointer group shadow-sm">
                       <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${item.done ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 group-hover:border-blue-400'}`}>
                          {item.done && <CheckCircle2 className="w-4 h-4" />}
                       </div>
                       <span className={`text-sm font-bold tracking-tight ${item.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{item.title}</span>
                    </div>
                  ))}
               </div>
            </div>
          ...
          {activeTab === 'docs' && (
            <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
               <label className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-slate-50/50 hover:bg-blue-50/50 hover:border-blue-300 transition-all cursor-pointer block relative">
                  ...
                  {uploading ? <RefreshCw className="w-10 h-10 text-blue-600 mx-auto mb-3 animate-spin" /> : <Upload className="w-10 h-10 text-slate-300 mx-auto mb-3" />}
                  <p className="text-sm font-bold text-slate-600 uppercase tracking-tight">{uploading ? 'Enviando...' : 'Arraste arquivos aqui'}</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">PDF, JPG, PNG (Max 10MB)</p>
               </label>
                    ...
                    <div key={i} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl hover:shadow-md transition-all group">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             ...
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{(doc.file_size / 1024 / 1024).toFixed(2)} MB</p>
                          </div>
                       </div>
                       <button className="p-2 text-slate-300 hover:text-blue-600 transition-all"><ChevronRight className="w-5 h-5" /></button>
                    </div>
                  ))}
                  {docs.length === 0 && !uploading && <p className="text-center text-xs text-slate-400 italic py-10">Nenhum documento anexado ainda.</p>}
               </div>
            </div>
          )}

          {activeTab === 'messages' && <div className="p-10 text-center text-xs text-slate-400 italic">Centralize aqui a conversa direta com o cliente deste processo.</div>}
          {activeTab === 'audit' && <div className="p-10 text-center text-xs text-slate-400 italic">Timeline real de cada ação tomada neste card.</div>}
        </main>
        ...
        <footer className="p-6 border-t border-slate-100 bg-white flex items-center justify-end gap-3 flex-shrink-0">
           ...
           <Button variant="primary" className="bg-blue-600 shadow-lg shadow-blue-200">Próxima Etapa</Button>
        </footer>
      </aside>
    </>
  );
