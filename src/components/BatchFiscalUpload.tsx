...
export function BatchFiscalUpload({ onClose, onComplete }: { onClose: () => void, onComplete: () => void }) {
    ...
    <div className="fixed inset-0 bg-adworks-dark/80 backdrop-blur-md z-[100] flex items-center justify-center p-4 lg:p-10 animate-in fade-in duration-300">
      <div className="bg-white rounded-[3rem] w-full max-w-5xl h-[85vh] shadow-2xl flex flex-col overflow-hidden border border-white/20">
        ...
        <div className="p-8 lg:p-12 border-b border-gray-100 flex items-center justify-between bg-adworks-gray/30">
          <div>
            <span className="bg-adworks-blue text-white px-3 py-1 rounded-full text-[9px] font-black tracking-widest uppercase">Operação de Lote</span>
            <h2 className="text-3xl font-black text-adworks-dark mt-3 italic uppercase tracking-tighter">Upload de Guias DAS</h2>
          </div>
          <button onClick={onClose} className="w-14 h-14 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-red-500 transition-all shadow-sm">
             <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 lg:p-12">
          {uploads.length === 0 ? (
            <label className="h-full border-4 border-dashed border-gray-100 rounded-[2.5rem] flex flex-col items-center justify-center group hover:border-adworks-blue/30 hover:bg-blue-50/30 transition-all cursor-pointer">
              <input type="file" multiple accept=".pdf" className="hidden" onChange={handleFileSelection} />
              ...
              <p className="text-xl font-black text-adworks-dark uppercase italic">Arraste as guias aqui</p>
              <p className="text-sm font-bold text-gray-400 mt-2 uppercase tracking-widest text-center max-w-xs">Selecione vários PDFs. O sistema tentará identificar os clientes automaticamente.</p>
            </label>
          ) : (
            <div className="space-y-4">
              {uploads.map((item) => (
                <div key={item.id} className="bg-adworks-gray/50 p-6 rounded-3xl border border-transparent hover:border-gray-200 transition-all flex items-center justify-between gap-6">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h4 className="font-bold text-adworks-dark text-sm leading-tight truncate max-w-[200px] md:max-w-md">{item.file.name}</h4>
                      <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${item.clientId ? 'text-green-600' : 'text-orange-500'}`}>
                        {item.clientId ? `Destino: ${item.clientName}` : 'Cliente não identificado ⚠️'}
                      </p>
                    </div>
                  </div>

                  <div className="flex items-center gap-4">
                    {item.status === 'uploading' && <Loader2 className="w-5 h-5 text-adworks-blue animate-spin" />}
                    {item.status === 'success' && <CheckCircle2 className="w-6 h-6 text-green-500" />}
                    {item.status === 'error' && <AlertCircle className="w-6 h-6 text-red-500" title={item.error} />}
                    {item.status === 'pending' && (
                      <button onClick={() => setUploads(prev => prev.filter(u => u.id !== item.id))} className="text-gray-300 hover:text-red-500 transition-colors">
                        <X className="w-5 h-5" />
                      </button>
                    )}
                  </div>
        ...
        <div className="p-8 lg:p-12 border-t border-gray-100 bg-white flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="text-center md:text-left">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Status da Operação</p>
              <p className="text-sm font-bold text-adworks-dark uppercase italic">
                {uploads.length} arquivos prontos • {uploads.filter(u => u.status === 'success').length} processados
              </p>
           </div>
           ...
           <div className="flex gap-4 w-full md:w-auto">
              ...
              <button 
                onClick={startUpload}
                disabled={processing || uploads.length === 0 || uploads.every(u => u.status === 'success')}
              ...
              >
                {processing ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckSquare className="w-5 h-5" />}
