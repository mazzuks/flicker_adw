      ...
      <div className="bg-[#F4F5F7] rounded-[2.5rem] w-full max-w-5xl h-[90vh] shadow-2xl flex flex-col overflow-hidden border border-white/20">
        ...
        <div className="p-8 lg:p-10 bg-white border-b border-gray-200 flex items-start justify-between relative overflow-hidden text-[#2D3E50]">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div>
              <h2 className="text-3xl font-black text-adworks-dark uppercase italic tracking-tighter leading-none mb-2">
                {task.title}
              </h2>
              <div className="flex items-center gap-3">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                  No grupo <span className="text-adworks-dark underline">{task.status}</span>
                </p>
          ...
          <button
            onClick={onClose}
            className="w-12 h-12 rounded-xl bg-adworks-gray text-gray-400 hover:text-red-500 transition-all flex items-center justify-center shadow-sm"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="bg-adworks-blue text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase shadow-lg shadow-blue-500/20"
                    >
                      Salvar
                    </button>
                    <button
                      onClick={() => setIsEditingDesc(false)}
                      className="text-gray-400 px-4 py-2 font-black text-[10px] uppercase"
                    >
                      Cancelar
                    </button>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-black px-2 py-0.5 rounded-md ${progress === 100 ? 'bg-green-500 text-white' : 'bg-adworks-gray text-gray-400'}`}
                >
                  {progress}%
                </span>
              </div>
              <div className="bg-adworks-gray/50 rounded-full h-2 overflow-hidden border border-white/50 shadow-inner">
                <div
                  className={`h-full transition-all duration-700 ${progress === 100 ? 'bg-green-500 shadow-[0_0_10px_#22c55e]' : 'bg-adworks-blue'}`}
              ...
              <div className="space-y-2 mt-4">
                {checklist.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 group">
                    ...
                    <span
                      className={`text-sm font-medium transition-all ${item.is_done ? 'text-gray-400 line-through opacity-50' : 'text-adworks-dark'}`}
                    >
                      {item.text}
                    </span>
                  </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    className="bg-adworks-dark text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 shadow-md"
                  >
                    Adicionar
                  </button>
                </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <div className="flex justify-end gap-2">
                    <button className="bg-adworks-blue text-white px-6 py-2 rounded-lg font-black text-[10px] uppercase shadow-lg shadow-blue-500/20">
                      Salvar Comentário
                    </button>
                  </div>
                </div>
          ...
          <div className="w-64 border-l border-gray-200 p-8 space-y-8 bg-[#F4F5F7] shrink-0 overflow-y-auto">
              ...
              <div className="relative">
                ...
                {showMemberPicker && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 z-50 animate-in slide-in-from-top-2">
                    <p className="text-[9px] font-black text-gray-400 uppercase mb-3 text-center">
                      Atribuir Membro
                    </p>
                    <div className="space-y-2">
                      {team.map((m) => (
                        <button
                          key={m.id}
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-[10px] font-bold text-adworks-dark border border-gray-100 flex items-center gap-2"
                        >
                          <div className="w-5 h-5 bg-adworks-blue/10 rounded-full flex items-center justify-center text-[8px] text-adworks-blue">
                    ...
                    <div className="space-y-2">
                      {['LOW', 'NORMAL', 'HIGH', 'URGENT'].map((p) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-[10px] font-black uppercase tracking-widest text-adworks-dark border border-gray-100 flex items-center gap-2"
                        >
                          <div
                            className={`w-2 h-2 rounded-full ${p === 'URGENT' ? 'bg-red-500' : p === 'HIGH' ? 'bg-orange-500' : 'bg-blue-500'}`}
                          />
                          {p}
                        </button>
                      ))}
              ...
              <div className="relative">
                ...
                {showDatePicker && (
                  <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 z-50 animate-in slide-in-from-top-2">
                    <p className="text-[9px] font-black text-gray-400 uppercase mb-4 text-center">
                      Definir Prazo
                    </p>
                    <input
                      type="date"
                      className="w-full bg-adworks-gray border-none rounded-xl px-4 py-3 text-xs font-bold mb-4 outline-none focus:ring-1 focus:ring-adworks-blue"
                    />
                    <button
                      onClick={() => setShowDatePicker(false)}
                      className="w-full bg-adworks-dark text-white py-2 rounded-xl text-[10px] font-black uppercase shadow-lg"
                    >
                      Salvar Data
                    </button>
                  </div>
            ...
            <div className="space-y-3">
              ...
              <label className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white border border-gray-100 hover:bg-adworks-gray hover:border-gray-200 transition-all group text-left cursor-pointer active:scale-95 shadow-sm">
                ...
                <span className="text-[10px] font-black text-gray-500 group-hover:text-adworks-dark uppercase tracking-widest">
                  Anexos
                </span>
                <input
                  type="file"
                  className="hidden"
                  onChange={() => alert('Função de anexo em lote sendo ligada ao Storage...')}
                />
              </label>
              <ActionButton
                icon={ImageIcon}
                label="Capa"
                onClick={() => alert('Escolha uma cor de destaque no Master Settings em breve.')}
              />
            </div>
            ...
            <div className="space-y-3 pt-8 border-t border-gray-200">
              ...
              <ActionButton
                icon={ChevronRight}
                label="Mover"
                onClick={() => alert('Arraste o card no tabuleiro para mover entre colunas.')}
              />
              <ActionButton
                icon={Copy}
                label="Copiar"
                onClick={() => alert('Duplicando processo...')}
              />
              <ActionButton
                icon={Zap}
                label="Automação"
                onClick={() => alert('Robô Adworks analisando o card...')}
              />
              <button
