...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] flex flex-col font-sans selection:bg-adworks-blue selection:text-white">
      <div
        className={`px-4 py-1 text-[9px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-2 text-white shrink-0 ${
          isMasterPath ? 'bg-orange-600' : isOperatorPath ? 'bg-[#1E293B]' : 'bg-adworks-blue'
        }`}
      >
        <Zap className="w-3 h-3 text-adworks-blue" />
        {isMasterPath
          ? 'Controle de Administração Master'
          : isOperatorPath
            ? 'Domínio de Força de Trabalho'
            : 'Domínio do Portal do Empreendedor'}
      </div>
      ...
      {isImpersonating && (
        <div className="bg-orange-600 text-white px-4 py-2 text-center text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-4 shadow-2xl z-[100] animate-in slide-in-from-top duration-500">
          ...
          <button
            ...
            className="bg-white text-orange-600 px-3 py-1 rounded font-black text-[9px]"
          >
            VOLTAR
          </button>
        </div>
          ...
          <div className="flex justify-between items-center h-full">
                  ...
                  return (
                    <Link
                      ...
                      className={`flex flex-col items-center justify-center px-4 h-[64px] transition-all relative group ${
                        isActive
                          ? 'bg-[#3D5266] text-white'
                          : 'text-white/60 hover:text-white hover:bg-white/5'
                      }`}
                    >
                      <item.icon
                        className={`w-5 h-5 mb-1 ${isActive ? 'text-white' : 'text-white/40 group-hover:text-white/60'}`}
                      />
                      <span className="text-[10px] font-bold uppercase tracking-tight">
                        {item.label}
                      </span>
                      {isActive && (
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-adworks-blue shadow-[0_-5px_10px_rgba(0,71,255,0.5)]"></div>
                      )}
                    </Link>
                  );
                })}
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 flex items-center justify-center text-white/60 hover:text-white transition-all">
                  <NotificationCenter />
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 pl-4 border-l border-white/10">
                <div className="text-right hidden sm:block">
                  ...
                  <p
                    className={`text-[9px] font-bold text-adworks-blue uppercase tracking-widest mt-1`}
                  >
                    {isMasterPath
                      ? 'Conta Master'
                      : isOperatorPath
