...
import { Building2, ArrowRight, Lock, Mail, ShieldCheck, UserCircle, Briefcase, Zap } from 'lucide-react';
    ...
    <div className="min-h-screen bg-adworks-gray flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full space-y-8 animate-in fade-in zoom-in duration-500">
        <div className="text-center">
          ...
          <h1 className="text-4xl font-black text-adworks-dark italic tracking-tighter uppercase mb-2">ADWORKS</h1>
          <p className="text-gray-400 font-bold text-xs uppercase tracking-[0.3em]">Access Protocol</p>
        </div>

        <div className="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden">
          <div className="p-10">
            <div className="mb-10">
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1">Login as :</label>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`p-2 rounded-xl bg-white shadow-sm ${isSelected ? 'text-adworks-blue' : 'text-gray-300'}`}>
                          <config.icon className="w-5 h-5" />
                        </div>
                        <div className="text-left">
                          <p className={`text-xs font-black uppercase tracking-widest ${isSelected ? 'text-adworks-blue' : ''}`}>{config.label}</p>
                          <p className="text-[10px] font-medium leading-tight opacity-60">{config.desc}</p>
                        </div>
                      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between px-2">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-adworks-blue focus:ring-adworks-blue" />
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover:text-adworks-dark transition-colors">Lembrar de mim</span>
                </label>
                <Link to="/forgot-password" size="sm" className="text-[10px] font-black text-adworks-blue uppercase tracking-widest hover:underline">Recuperar Senha</Link>
              </div>
          ...
          <div className="bg-adworks-gray py-6 px-10 border-t border-gray-50 flex items-center justify-between">
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest italic">Encrypted Connection</span>
            <Link to="/register" className="text-[10px] font-black text-adworks-blue uppercase tracking-widest hover:underline">Criar Nova Conta</Link>
          </div>
        </div>
      </div>
    </div>
