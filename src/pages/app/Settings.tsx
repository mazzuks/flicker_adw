    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <Card className="border border-slate-200 shadow-sm overflow-hidden" noPadding>
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <Button
            size="sm"
            onClick={executeSmokeTest}
            isLoading={loading}
            variant="secondary"
            className="gap-2"
          >
            <RefreshCw className="w-4 h-4" /> Executar Diagnóstico
          </Button>
        </div>
        <div className="p-6">
          {!testResults ? (
            <p className="text-xs text-slate-400 font-medium italic">
              Execute o diagnóstico para validar a integridade do banco de dados.
            </p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatusItem label="Funções RPC" success={testResults.rpc} />
              <StatusItem label="RLS & Segurança" success={testResults.rls} />
            </div>
          )}
      ...
      <Card className="border-dashed border-blue-200 bg-blue-50/30 p-8 rounded-2xl">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h3 className="font-bold text-slate-900 uppercase tracking-tight text-sm italic">
              Ambiente de Desenvolvimento
            </h3>
            <p className="text-xs text-slate-500 font-medium">Reset e população de dados seed.</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          <Button
            ...
            isLoading={loading}
            variant="primary"
            className="bg-blue-600 shadow-lg shadow-blue-200 px-8"
          >
...
function StatusItem({ label, success }: { label: string; success: boolean }) {
  return (
    <div
      className={`p-4 rounded-xl border flex flex-col items-center gap-3 text-center transition-all ${success ? 'bg-white border-slate-100 shadow-sm' : 'bg-red-50 border-red-100'}`}
    >
      {success ? (
      ...
      <span
        className={`text-[9px] font-black uppercase tracking-widest ${success ? 'text-slate-400' : 'text-red-500'}`}
      >
        {label}
      </span>
    </div>
