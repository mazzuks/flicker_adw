  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1600px] mx-auto px-4 lg:px-8">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 pb-6 border-b border-adworks-border">
        <div>
          <h1 className="text-2xl font-bold text-adworks-dark tracking-tight">
            Carteira de Clientes
          </h1>
          <p className="text-adworks-muted text-sm mt-1">
            Gestão de empresas e acessos operacionais.
          </p>
        </div>
            ...
            <thead className="bg-adworks-gray border-b border-adworks-border">
              <tr>
                <th className="px-6 py-4 w-10">
                  <input
                    type="checkbox"
                    className="w-4 h-4 rounded border-adworks-border text-adworks-blue focus:ring-adworks-blue"
                  />
                </th>
                <th className="px-4 py-4 text-[10px] font-black text-adworks-muted uppercase tracking-widest">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-adworks-muted uppercase tracking-widest text-right">
                  Ações
                </th>
              </tr>
            </thead>
              ...
              {filteredCompanies.map((c) => (
                <tr key={c.id} className="group hover:bg-adworks-accent/50 transition-all">
                  ...
                  <td className="px-4 py-4">
                    ...
                    <p className="text-[10px] font-medium text-adworks-muted uppercase mt-0.5 tracking-tighter">
                      {c.name}
                    </p>
                  </td>
                  <td className="px-4 py-4">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${c.status === 'ACTIVE' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-orange-50 text-orange-700 border-orange-100'}`}
                    >
                      {c.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                      <button
                        ...
                        title="Impersonate"
                        className="p-2 rounded-lg bg-adworks-surface border border-adworks-border text-adworks-muted hover:text-orange-600 hover:border-orange-100 transition-all"
                      >
                        <LogIn className="w-4 h-4" />
                      </button>
                      <button className="p-2 rounded-lg bg-adworks-surface border border-adworks-border text-adworks-muted hover:text-adworks-blue hover:border-blue-100 transition-all">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button className="p-2 rounded-lg bg-adworks-surface border border-adworks-border text-adworks-muted hover:text-adworks-dark hover:border-adworks-border transition-all">
