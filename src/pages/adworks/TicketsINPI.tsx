...
export function TicketsINPI() {
  ...
  const statusCounts = {
    ...
    IN_PROGRESS: tickets.filter((t) => t.status === 'IN_PROGRESS' || t.status === 'SUBMITTED')
      .length,
            ...
            <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-purple-50/30">
              <div>
                ...
                <h2 className="text-2xl font-black text-adworks-dark mt-2 italic uppercase tracking-tighter">
                  {selectedTicket.data_json?.brand_name || selectedTicket.client?.fantasy_name}
                </h2>
              </div>
              <button
                  ...
                  <div className="space-y-3">
                    <p className="text-sm">
                      ...
                      <span className="font-black text-adworks-dark">
                        {selectedTicket.data_json?.brand_name || 'N/A'}
                      </span>
                    </p>
                    <p className="text-sm">
                      <span className="text-gray-400 font-bold uppercase text-[9px] tracking-widest">
                      ...
                      <span className="font-black text-adworks-dark uppercase italic">
                        Classe {selectedTicket.data_json?.brand_class || 'N/A'}
                      </span>
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-adworks-gray/50 rounded-3xl p-6 border border-gray-100">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[10px] font-black uppercase px-2 py-1 rounded-full border ${getStatusColor(selectedTicket.status)}`}
                      >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium tracking-tight">
            Gestão de protocolos INPI e proteção de propriedade intelectual.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="group hover:bg-adworks-gray/30 transition-all">
                    ...
                    <td className="px-8 py-6">
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest ${getStatusColor(ticket.status)} bg-opacity-20 border-none`}
                      >
