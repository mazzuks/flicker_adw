      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-gray-400 font-bold text-xs uppercase tracking-widest mt-2">
            Gerenciamento dinâmico de processos operacionais.
          </p>
        </div>
      </div>
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex-1 flex gap-6 overflow-x-auto pb-6 scrollbar-hide">
          {COLUMNS.map((column) => (
            <div
              key={column.id}
              className="w-[320px] shrink-0 flex flex-col bg-[#EBEEF0] rounded-[2rem] p-4 border border-transparent hover:border-slate-300 transition-colors shadow-inner"
            >
              <div className="flex items-center justify-between px-4 py-3 mb-4">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${column.color}`}></div>
                  <h3 className="font-black text-adworks-dark uppercase tracking-tighter text-sm italic">
                    {column.title}
                  </h3>
                </div>
                          ...
                          {(provided, snapshot) => (
                            ...
                            >
                              <div className="flex items-start justify-between mb-4">
                                <div
                                  className={`text-[8px] font-black px-2 py-0.5 rounded-full text-white uppercase tracking-widest ${getPriorityStyle(task.priority)}`}
                                >
                                  {task.priority}
                                </div>
