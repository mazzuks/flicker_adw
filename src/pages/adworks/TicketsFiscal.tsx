...
export default function TicketsFiscal() {
  ...
  const getStatusBadge = (status: string) => {
    const configs: Record<string, any> = {
      ...
      WAITING_CLIENT: { label: 'Aguard. Cliente', color: 'text-blue-600 bg-blue-50 border-blue-100' },
    };
    const config = configs[status] || { label: status, color: 'text-gray-400 bg-gray-50 border-gray-100' };
    return (
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-adw-soft border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Guias Pendentes</p>
          <h3 className="text-4xl font-black tracking-tighter italic text-orange-500">
            {tickets.filter(t => t.status === 'NEW').length}
          </h3>
        </div>
        <div className="bg-white p-8 rounded-[2.5rem] shadow-adw-soft border border-gray-100">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Faturamento do Mês</p>
          <h3 className="text-4xl font-black tracking-tighter italic text-adworks-dark">R$ 12.450</h3>
        </div>
        <div className="bg-adworks-dark p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
           ...
           <p className="text-[10px] font-black text-blue-200 uppercase tracking-widest mb-2">SLA Médio Fiscal</p>
         ...
         <table className="w-full text-left">
           <thead>
             <tr className="bg-adworks-gray/50 border-b border-gray-100">
               <th className="px-8 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Empresa</th>
             ...
             {filteredTickets.map((ticket) => (
               <tr key={ticket.id} className="group hover:bg-adworks-gray/30 transition-all">
                 <td className="px-8 py-6">
                    <p className="font-black text-adworks-dark uppercase italic tracking-tight">{ticket.client_name}</p>
                    <p className="text-[10px] font-bold text-gray-400 uppercase">Abertura: {new Date(ticket.created_at).toLocaleDateString('pt-BR')}</p>
                 </td>
                 <td className="px-8 py-6">
                 ...
                 <td className="px-8 py-6">
                    <p className="font-black text-adworks-dark tracking-tighter">R$ {ticket.amount?.toFixed(2)}</p>
                 </td>
                 <td className="px-8 py-6">
