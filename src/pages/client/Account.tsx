...
import { User, Lock, Building2, Bell, Save, CreditCard, ChevronRight, Mail, Phone, Camera } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      <div>
        ...
        <p className="text-gray-500 font-medium">Gerencie suas informações pessoais e configurações.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-[2.5rem] p-10 shadow-adw-soft border border-gray-100 min-h-[550px] relative">
          {message.text && (
            <div className={`mb-8 p-5 rounded-2xl text-sm font-black uppercase tracking-tight flex items-center gap-3 animate-in slide-in-from-top-2 ${message.type === 'success' ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
          ...
          {activeTab === 'profile' && (
            <div className="space-y-10">
              <div className="flex items-center gap-8 pb-10 border-b border-gray-50">
                 ...
                 <div>
                    <h3 className="text-xl font-black text-adworks-dark uppercase italic leading-tight">{profileData.full_name || 'Seu Nome'}</h3>
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mt-1">{profileData.email}</p>
                 </div>
              </div>
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
              <div className="p-6 bg-orange-50 border border-orange-100 rounded-3xl flex gap-4">
                 ...
                 <div>
                    <p className="text-sm font-black text-orange-700 uppercase tracking-tighter mb-1">Dica de Segurança</p>
                    <p className="text-xs font-medium text-orange-600 leading-relaxed">Use uma senha forte com letras, números e símbolos para proteger os dados da sua empresa.</p>
                 </div>
              </div>
              ...
              <form onSubmit={handlePasswordUpdate} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Confirmar Nova Senha</label>
                    <input
                      type="password"
          ...
          {activeTab === 'subscription' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <div className="bg-adworks-dark p-10 rounded-[2rem] text-white relative overflow-hidden group">
                ...
                <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-4">Plano de Serviço Ativo</p>
                <h3 className="text-5xl font-black italic uppercase tracking-tighter mb-6">{subscription?.plan?.name || 'ADWORKS BASIC'}</h3>
                <div className="flex items-center gap-4">
                  <div className="bg-white/10 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10">
                    <span className="text-xl font-black italic">R$ {subscription?.plan?.price || '197,00'}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="p-6 bg-adworks-gray rounded-2xl border border-transparent flex justify-between items-center group cursor-pointer hover:bg-white hover:border-adworks-blue/20 transition-all">
                   <div>
                     ...
                     <p className="font-black text-adworks-dark group-hover:text-adworks-blue transition-colors uppercase italic">PIX Recorrente</p>
                   </div>
                   <ChevronRight className="w-4 h-4 text-gray-300" />
                 </div>
                 <div className="p-6 bg-adworks-gray rounded-2xl border border-transparent flex justify-between items-center group cursor-pointer hover:bg-white hover:border-adworks-blue/20 transition-all">
                   <div>
                     <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Próximo Vencimento</p>
                     <p className="font-black text-adworks-dark group-hover:text-adworks-blue transition-colors uppercase italic">10 MAR 2026</p>
                   </div>
                   <ChevronRight className="w-4 h-4 text-gray-300" />
                 </div>
              </div>
          ...
          {activeTab === 'notifications' && (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-20 opacity-30">
               ...
               <p className="font-black text-adworks-dark uppercase italic tracking-tighter">Configurações de Alerta em breve</p>
            </div>
          )}
        </div>
      </div>
...
function AlertCircle(props: any) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>
  )
}
