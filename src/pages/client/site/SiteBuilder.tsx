...
const BLOCK_LIBRARY = [
  {
    ...
    defaultContent: {
      ...
      subtitle: 'Uma breve descrição impactante.',
      cta: 'Saiba Mais',
    },
  },
  {
    ...
    defaultContent: {
      items: [
        { title: 'Fácil', desc: 'Simples de usar' },
        { title: 'Rápido', desc: 'Velocidade total' },
      ],
    },
  },
  {
    ...
    label: 'Sobre Nós',
    icon: Type,
    defaultContent: { title: 'Nossa História', text: 'Conte como tudo começou...' },
  },
  {
    type: 'pricing',
    label: 'Preços',
    icon: Globe,
    defaultContent: { plans: [{ name: 'Básico', price: 'R$ 99' }] },
  },
  {
    type: 'contact',
    label: 'Contato/Form',
    icon: MousePointer2,
    defaultContent: { title: 'Fale Conosco', email: true, phone: true },
  },
];
...
export function SiteBuilder() {
  ...
  const [sections, setSections] = useState<any[]>([
    {
      ...
      type: 'hero',
      content: {
        title: 'NOVA EMPRESA ADWORKS',
        subtitle: 'Site gerado automaticamente.',
        cta: 'CONHECER',
      },
    },
  ]);
  ...
  const [activeSidePanel, setActiveSidePanel] = useState<
    'blocks' | 'navigator' | 'theme' | 'seo' | null
  >(null);
    ...
    <div className="fixed inset-0 z-[200] flex flex-col bg-[#0F172A] font-sans selection:bg-adworks-blue selection:text-white">
      ...
      <div className="h-14 bg-[#1E293B] border-b border-white/5 flex items-center justify-between px-4 shrink-0 z-[100]">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1 bg-white/5 p-1 rounded-xl">
            <button
              ...
              className={`p-2 rounded-lg transition-all ${previewMode === 'desktop' ? 'bg-adworks-blue text-white shadow-lg' : 'text-white/30 hover:text-white'}`}
            >
              <Monitor className="w-4 h-4" />
            </button>
            <button
              onClick={() => setPreviewMode('tablet')}
              className={`p-2 rounded-lg transition-all ${previewMode === 'tablet' ? 'bg-adworks-blue text-white shadow-lg' : 'text-white/30 hover:text-white'}`}
            >
              <Layers className="w-4 h-4" />
            </button>
            <button
              onClick={() => setPreviewMode('mobile')}
              className={`p-2 rounded-lg transition-all ${previewMode === 'mobile' ? 'bg-adworks-blue text-white shadow-lg' : 'text-white/30 hover:text-white'}`}
            >
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="text-white/60 hover:text-white p-2 transition-all">
            <Eye className="w-5 h-5" />
          </button>
          <button className="bg-adworks-blue text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:brightness-110 active:scale-95 transition-all">
            Publish Site
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="w-16 bg-[#2D3E50] flex flex-col items-center py-6 gap-6 border-r border-white/5 z-50 shadow-2xl">
          {[
            ...
            <button
              key={tool.id}
              onClick={() =>
                setActiveSidePanel(activeSidePanel === tool.id ? null : (tool.id as any))
              }
        ...
        {activeSidePanel && (
          <div className="absolute left-16 top-0 bottom-0 w-[300px] bg-[#1E293B] text-white border-r border-white/10 shadow-2xl z-40 animate-in slide-in-from-left duration-300">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-white/60 italic">
                {activeSidePanel}
              </h3>
              <button
                            ...
                            {(provided) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div {...provided.dragHandleProps} className="opacity-20">
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-[#0F172A] relative">
          <div className="flex-1 overflow-y-auto p-12 lg:p-20 flex justify-center custom-scrollbar">
            ...
            <div
              className={`bg-white transition-all duration-700 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative h-fit ${
                previewMode === 'desktop'
                  ? 'w-full max-w-[1200px] min-h-[1400px] rounded-sm'
                  : previewMode === 'tablet'
                    ? 'w-[768px] min-h-[1400px] rounded-xl'
                  ...
                  <div className="p-16 lg:p-32 text-center">
                    {section.type === 'hero' && (
                      <div className="max-w-2xl mx-auto space-y-8 pointer-events-none">
                        <h1 className="text-5xl lg:text-8xl font-black text-[#1E293B] tracking-tighter uppercase italic leading-[0.85]">
                          {section.content.title}
                        </h1>
                        <p className="text-base lg:text-xl text-slate-400 font-medium leading-relaxed italic tracking-tight">
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="space-y-5">
                  <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] flex items-center gap-2">
                    <Box className="w-3.5 h-3.5" /> Layout
                  </p>
                  <div className="space-y-3">
                    <label className="text-[8px] font-black text-white/20 uppercase tracking-widest ml-1">
                      Display Mode
                    </label>
                    <div className="grid grid-cols-2 gap-2 bg-black/20 p-1 rounded-xl">
                      <button className="bg-[#2D3E50] text-white py-2 rounded-lg text-[9px] font-black uppercase shadow-sm">
                        Block
                      </button>
                      <button className="text-white/30 py-2 rounded-lg text-[9px] font-black uppercase">
                        Flex
                      </button>
                    </div>
                  </div>
                ...
                <div className="space-y-5 pt-8 border-t border-white/5">
                  <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] flex items-center gap-2">
                    <Maximize2 className="w-3.5 h-3.5" /> Spacing (px)
                  </p>
                    ...
                    <div className="flex flex-col items-center gap-4">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <input
                          type="text"
                          defaultValue="120"
                          className="w-12 bg-[#2D3E50] border-none rounded-lg p-2 text-center text-[10px] font-black text-adworks-blue outline-none focus:ring-1 focus:ring-adworks-blue"
                        />
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col items-center gap-1">
                          <span className="text-[7px] font-black text-white/10 uppercase">
                            Left
                          </span>
                          <input
                            type="text"
                            defaultValue="0"
                            className="w-10 bg-[#2D3E50] border-none rounded-lg p-2 text-center text-[10px] font-black text-white/40 outline-none"
                          />
                        </div>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <input
                          type="text"
                          defaultValue="120"
                          className="w-12 bg-[#2D3E50] border-none rounded-lg p-2 text-center text-[10px] font-black text-adworks-blue outline-none"
                        />
                      </div>
                    </div>
                  </div>
